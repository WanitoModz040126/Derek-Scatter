...
function buildInitialGrid() {
  // purely decorative first paint, real grid comes from first spin
...
function renderGrid(grid, winningIcons = new Set(), fallingCells = null) {
  reelGrid.innerHTML = '';
  const cols = CONFIG.cols, rows = CONFIG.rows;
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      if (fallingCells && fallingCells.has(`${c}-${r}`)) tile.classList.add('falling');
...
async function animateStep(step, isFreeSpin = false) {
...
async function doSpin() {
  ...
  try {
    ...
    credits = data.credits - 0; // server-authoritative balance (post bet + post win)
    // Show deducted balance immediately for feedback, animate up as wins land
    ...
    if (data.freeSpinRounds && data.freeSpinRounds.length > 0) {
      freeSpinsLeftEl.textContent = data.freeSpinRounds.length;
